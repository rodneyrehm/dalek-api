...
Unit._wrapForQueue = function(name, callback, options) {
  return function() {
    ...
    unit.emit('log', 'debug', 'action.scheduling', name);
    ...
    queuedItem.run(function() {
      unit.emit('log', 'debug', 'action.preparing', name);
      ...
      try {
        ...
        unit.emit('log', 'debug', 'action.executing', args.slice(2));
...
Unit.prototype.queue = function() {
  ...
  deferred.promise.then(function(result) {
    ...
    this.emit('log', 'debug', 'action.passing', this._tasks[this._items].name);
    this._tasks[this._items].state = true;
    ...
    this.emit('log', 'debug', 'action.failing', this._tasks[this._items].name);
    this._tasks[this._items].state = null;
...
console.log("---- done()", this._queue.length);
  this._item = this._queue.shift();
  if (this._item) {
    this._items++;
    this._item();
  } else {
    console.log("yeah, Im done")
...
Unit.prototype.initialize = function(parent) {
  ...
  // task status for reporting
  this._tasks = [];

  // task scheduling for execution
  this._queue = [];
  this._items = -1;

  // unit status for execution (abort following tests)
